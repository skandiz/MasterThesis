...
if 0:
    ...
    tracking_path = "../tracking/25b_25r/"
...
frames = np.arange(0, 100000, 1) #rawTrajs.frame.unique().astype(int)
nFrames = len(frames)
...
if 0:
    ...
    def update_graph(frame):
        df = rawTrajs.loc[(rawTrajs.frame == frame) , ["x", "y", "color", "r"]]
        for i in range(50):
    ...
    df = rawTrajs.loc[(rawTrajs.frame == 0), ["x", "y", "color", "r"]]
    graph = []
    for i in range(50):
...
if 1:
    ...
    ani = matplotlib.animation.FuncAnimation(fig, update_graph, np.arange(0, 100000, 5), interval = 5, blit=False)
    writer = matplotlib.animation.FFMpegWriter(fps = 30, metadata = dict(artist='Matteo Scandola'), extra_args=['-vcodec', 'libx264'])
