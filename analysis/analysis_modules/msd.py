...
x = np.arange(1., 100.1, .1) # range of power law fit

##############################################################################################################
#                                                GLOBAL IMSD                                                 #
##############################################################################################################
print("Global IMSD")
imsd, fit, pw_exp, _, _ = get_imsd(rawTrajs, pxDimension, fps, maxLagtime, nDrops)
imsd_smooth, fit_smooth, pw_exp_smooth, _, _ = get_imsd(smoothTrajs, pxDimension, fps, maxLagtime, nDrops)
...
if plot_verb:
    ...
    plt.tight_layout()
    if save_verb: plt.savefig(f"./{res_path}/mean_squared_displacement/IMSD_smooth.png", bbox_inches='tight')
    if show_verb: 
        plt.show()
    else:
        plt.close()   
...
##############################################################################################################
print("Global EMSD")
MSD_b, MSD_r, fit, _ = get_emsd(imsd, x, red_particle_idx, nDrops)
# Raw Trajs: temp variable to print pw_exp results
...
MSD_b_smooth, MSD_r_smooth, fit_smooth, _ = get_emsd(imsd_smooth, x, red_particle_idx, nDrops)
# Smooth Trajs: temp variable to print pw_exp results
...
if plot_verb:
    ...
    ax.plot(imsd_smooth[1:].index, fit_smooth["fit_r"], 'r--')
    ax.set(xscale = 'log', yscale = 'log', ylabel = r'$\langle \Delta r^2 \rangle$ [$px^2$]',   
            xlabel = 'lag time $t$ [s]', title = "EMSD - Smooth Trajectories")
    ax.legend()
    ax.grid()
    if save_verb: plt.savefig(f"./{res_path}/mean_squared_displacement/EMSD_smooth.png", bbox_inches='tight')
    if show_verb:
        plt.show()
    else:
        plt.close()
...
if run_windowed_analysis:
    MSD_wind, fit_wind, pw_exp_wind, _, _ = get_imsd_windowed(nSteps, startFrames, endFrames, rawTrajs, pxDimension, fps, maxLagtime, nDrops)

    MSD_wind_smooth, fit_wind_smooth, pw_exp_wind_smooth, _, _ = get_imsd_windowed(nSteps, startFrames, endFrames, smoothTrajs, 
                                                                            pxDimension, fps, maxLagtime, nDrops)
    for k in range(nSteps):
        MSD_wind[k].columns = [str(i) for i in range(nDrops)]
        MSD_wind[k].to_parquet(f"./{analysis_data_path}/MSD/raw/MSD_wind_raw{k}.parquet")
        MSD_wind_smooth[k].columns = [str(i) for i in range(nDrops)]
        MSD_wind_smooth[k].to_parquet(f"./{analysis_data_path}/MSD/smooth/MSD_wind_smooth{k}.parquet")
else:
    try:
        MSD_wind = []
        MSD_wind_smooth = []
        for k in range(nSteps):
            temp = pd.read_parquet(f"./{analysis_data_path}/MSD/raw/MSD_wind_raw{k}.parquet")
            temp.columns = [i for i in range(nDrops)]
            MSD_wind.append(temp)
            temp = pd.read_parquet(f"./{analysis_data_path}/MSD/smooth/MSD_wind_smooth{k}.parquet") 
            temp.columns = [i for i in range(nDrops)]
            MSD_wind_smooth.append(temp)
    except:
        raise Exception("Windowed MSD data not found. Set run_windowed_analysis to True to generate it.")
    ...
    if plot_verb:
        if animated_plot_verb:
            ...
            if save_verb: ani.save(f'./{res_path}/mean_squared_displacement/windowed_analysis/IMSD_wind_raw.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
            if show_verb:
                plt.show()
            else:
                plt.close()
            ...
            fig.canvas.mpl_connect('button_press_event', onClick)
            ani = matplotlib.animation.FuncAnimation(fig, update_graph, nSteps, interval = 5, blit=False)
            if save_verb: ani.save(f'./{res_path}/mean_squared_displacement/windowed_analysis/IMSD_wind_smooth.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
            if show_verb:
                plt.show()
            else:
                plt.close()
...
print("Windowed EMSD")
EMSD_wind_b, EMSD_wind_r, fit_dict, _ = get_emsd_windowed(MSD_wind, x, nDrops, red_particle_idx, nSteps, maxLagtime)
EMSD_wind_b_smooth, EMSD_wind_r_smooth, fit_dict_smooth, _ = get_emsd_windowed(MSD_wind_smooth, x, nDrops, red_particle_idx, nSteps, maxLagtime)
...
if plot_verb:
    ...
    if save_verb: plt.savefig(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_beta_raw.png', bbox_inches='tight')
    if show_verb:
        plt.show()
    else:
        plt.close()
    ...
    ax.grid()
    ax.set(xlabel = 'Window time [s]', ylabel = r'$\beta$', ylim = (0, 2))
    if save_verb: plt.savefig(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_beta_smooth.png', bbox_inches='tight')
    if show_verb:
        plt.show()
    else:
        plt.close()
    ...
    plt.tight_layout()
    if save_verb: plt.savefig(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_D_raw.png', bbox_inches='tight')
    if show_verb:
        plt.show()
    else:
        plt.close()
    ...
    ax1.grid()
    plt.tight_layout()
    if save_verb: plt.savefig(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_D_smooth.png', bbox_inches='tight')
    if show_verb:
        plt.show()
    else:
        plt.close()
    ...
    if animated_plot_verb:
        ...
        fig.canvas.mpl_connect('button_press_event', onClick)
        ani = matplotlib.animation.FuncAnimation(fig, update_graph, nSteps, interval = 5, blit=False)
        if save_verb: ani.save(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_wind_raw.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
        if show_verb:
            plt.show()
        else:
            plt.close()
        ...
        fig.canvas.mpl_connect('button_press_event', onClick)
        ani = matplotlib.animation.FuncAnimation(fig, update_graph, nSteps, interval = 5, blit=False)
        if save_verb: ani.save(f'./{res_path}/mean_squared_displacement/windowed_analysis/EMSD_wind_smooth.mp4', fps=30, extra_args=['-vcodec', 'libx264'])
        if show_verb:
            plt.show()
        else:
            plt.close()