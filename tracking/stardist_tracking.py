...
if test_verb: 
    n_samples = 1000
    test_detection_df = test_detection(n_samples, n_frames, nDrops, video_selection, model, model_name, video, xmin, ymin, xmax, ymax, w, h, resolution, save_path)
...
if detect_verb:
    ...
    plt.tight_layout()
    plt.savefig(save_path + f'raw_instances_{model_name}_{startFrame}_{endFrame}.png', dpi = 500)
    plt.close()
else:
    raw_detection_df = pd.read_parquet(save_path + f'raw_detection_{video_selection}_{model_name}_{startFrame}_{endFrame -1}.parquet')
...
if link_verb:
    if merge_present:
        ...
        trajectories_pre_merge = tp.filter_stubs(t, 250)
        ...
        t = t.sort_values(['frame', 'particle'])
        trajectories_post_merge = tp.filter_stubs(t, 250)
        # CREATE COLOR COLUMN AND SAVE DF
        ...
        t = t.sort_values(['frame', 'particle'])
        trajectories = tp.filter_stubs(t, 250)
        # CREATE COLOR COLUMN AND SAVE DF
        ...
        trajectories.to_parquet(save_path + f'raw_tracking_{video_selection}_{model_name}_{startFrame}_{endFrame}.parquet', index = False)
else:
    if merge_present:
        ...
        print('Importing linked trajectories...')
        trajectories = pd.read_parquet(save_path + f'raw_tracking_{video_selection}_{model_name}_{startFrame}_{endFrame}.parquet')
    

print(trajectories.particle.unique())
